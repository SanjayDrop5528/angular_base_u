...
export class MasterSingleDetailFormComponent {
  ...
  addForm(mode: any) {
    ...
    if (this.isPopupEdit) {
      ...
      if (this.config?.addSubjectId == true) {
        data[this.config?.mapColumnfield] = this.model[this.config.localSetField]
      }
    ...
    } else if (this?.config?.detailForm?.isPageEdit) {
      // this.router.navigate([`${this.config.addRoute}`])
      this.IsShow = false;
      this.config.ScreenMode = "page"
      this.router.navigate(["data/add/page/question/7cf5db17af894fb19524c7a756886dbb"])
  ...
  SaveProjectTeam() {
    if (!this.detailForm.valid) {
      const invalidLabels: any = this.helperService.getDataValidation(this.detailForm.controls);
  ...
  onActionButtonClick($event: any, item: any, data: any) {
    ...
    if (item.formAction == "listpopup") {
      ...
      this.dialogService.confirmationBox(this.translateService.instant('CONFIRM_MSG.DELETE_RECORD')).afterClosed().subscribe((confirm) => {
        if (!confirm) return;
        this.dataService.deleteDataById(this.config.detailForm.collectionName, data._id).subscribe(
          (res: any) => {
            this.dialogService.openSnackBar(
              this.translateService.instant('SUCCESS_MSG.DELETE_SUCCESS'),
  ...
  saveChild(config: any, data: any, value?: any) {
    ...
    if (!this.valueformGrupo.valid) {
      const invalidLabels: any = this.helperService.getDataValidation(this.valueformGrupo.controls);
      this.dialogService.openSnackBar("Error in " + invalidLabels, "OK");
  ...
  delete_record($event: any, row: any, config: any) {
    $event.preventDefault();
    $event.stopPropagation();
    this.dialogService.confirmationBox(this.translateService.instant('CONFIRM_MSG.DELETE_DATA')).afterClosed().subscribe((confirm) => {
      if (!confirm) return;
      ...
      this.dataService.deleteDataById(config.collectionName, row._id).subscribe((res: any) => {
        ...
        this.dialogService.openSnackBar(
          this.translateService.instant('SUCCESS_MSG.ARTICLE_DELETED'),
          this.translateService.instant('FORM.COMMON.OK')
        );
